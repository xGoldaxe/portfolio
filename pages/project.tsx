...
export default function Home({ handleTransition, transitionOver }) {
  ...
  let cancelablePromise: null | any = null;
  ...
  async function updateProject(e, inTransition) {
    if (
      !inTransition &&
      ((e.deltaY > 0 && actualProject < ALLPROJECTS.length - 1) ||
        (e.deltaY < 0 && actualProject > 0))
    ) {
  ...
  async function handleNewProject(direction, inTransition) {
    if (
      !inTransition &&
      (actualProject < ALLPROJECTS.length - 1 || actualProject > 0)
    ) {
      setInTransition(true);
      await projectTransiton();
      setInTransition(false);
        ...
        <div className="projects">
          <div className="projects__info">
            <div className="projects__info__scroller">
              {actualProject > 0 ? (
                <div
                  className="projects__info__scroller__previous"
                  onClick={() => handleNewProject("-", inTransition)}
                >
                  <p>previous</p>
            ...
            <div className="projects__info__desc">
              <div className="projects__info__desc__textContainer">
                <p className="projects__info__desc__textContainer__number">
                  {"0" + (actualProject + 1)}
                </p>
              </div>
              <div className="projects__info__desc__textContainer">
                <h3
                  className="projects__info__desc__textContainer__title"
                  onClick={() =>
                    handleTransition(
                      `project/${ALLPROJECTS[actualProject].name
                        .toLowerCase()
                        .split(" ")
                        .join("_")}`,
                      "transition1"
              ...
              <div className="projects__info__desc__textContainer">
                <p className="projects__info__desc__textContainer__date">
                  {ALLPROJECTS[actualProject].date}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="projects__image--container">
            ...
            >
              <div className="projects__image__container">
                <img
                  src={`/image/${ALLPROJECTS[actualProject].image}`}
                  alt="project"
                />
              </div>
